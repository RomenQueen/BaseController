...
public class EaseContactAdapter extends ArrayAdapter<EaseUser> implements SectionIndexer {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.visit = (TextView) convertView.findViewById(R.id.tv_visit);
        ...
        if (user == null) {
            Log.d("ContactAdapter", position + " null");
            return convertView;
        }
        String username = user.getUsername();
        String header = user.getInitialLetter();
        holder.nameView.setText(username);
        holder.visit.setTextColor(Color.parseColor("#686868"));
        if (position == 0 || header != null && !header.equals(getItem(position - 1).getInitialLetter())) {
            if (TextUtils.isEmpty(header)) {
                holder.headerView.setVisibility(View.GONE);
            } else {
                if (isShowLetter()) {
                    LOG.e("EaseContactAdapter", position + "LINE:header = " + header);
